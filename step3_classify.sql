-- STEP 3: READ-ONLY classification report. No DELETE/UPDATE/INSERT anywhere in this script.
\pset pager off
...
canonical AS (
  SELECT
    *,
    lower(trim(customer_part_no)) AS cpn_norm,
    -- customer_part_no is ambiguous (maps to 2 different real parts) only for these two values;
    -- disambiguate those two using cavity_count, otherwise key on customer_part_no alone.
...
part_hist AS (
  SELECT
    ...
    p.active, p.cavity_count, p.part_weight_g,
    (SELECT count(*) FROM production_entries pe WHERE pe.part_id = p.id) AS prod_entries,
    (SELECT count(*) FROM bags b WHERE b.part_id = p.id) AS bags,
    (SELECT count(*) FROM machine_assignments ma WHERE ma.part_id = p.id) AS mach_assign,
    (SELECT count(*) FROM fpa_submissions f WHERE f.part_id = p.id) AS fpa,
...
part_keyed AS (
  SELECT
    ph.*,
    (ph.prod_entries + ph.bags + ph.mach_assign + ph.fpa) AS hist_total,
    CASE WHEN ph.cpn_norm IN ('f364-cb5aa-01', 'hc443qqvba02')
         THEN ph.cpn_norm || '|cav' || ph.cavity_count::text
         ELSE ph.cpn_norm
    END AS canon_key
  FROM part_hist ph
),
matched AS (
  SELECT pk.*, c.shrp_part_code AS canon_shrp, c.part_name AS canon_part_name,
         c.cavity_count AS canon_cavity, c.part_weight_g AS canon_weight, c.canon_key AS c_key
  FROM part_keyed pk
  JOIN canonical c ON c.canon_key = pk.canon_key
),
unmatched AS (
  SELECT pk.* FROM part_keyed pk
  WHERE NOT EXISTS (SELECT 1 FROM canonical c WHERE c.canon_key = pk.canon_key)
),
matched_ranked AS (
  SELECT
    m.*,
    count(*) OVER (PARTITION BY c_key) AS group_size,
    count(*) FILTER (WHERE hist_total > 0) OVER (PARTITION BY c_key) AS group_hist_count,
    row_number() OVER (
      PARTITION BY c_key
      ORDER BY
        (hist_total > 0) DESC,
        (part_code ILIKE 'SHRP-P%') DESC,
        id ASC
    ) AS pick_rank
  FROM matched m
)
SELECT
  id, part_code, canon_shrp AS erp_shrp_code, customer_part_no, active,
  cavity_count, canon_cavity, part_weight_g, canon_weight,
  hist_total,
  CASE
    WHEN group_hist_count > 1 AND hist_total > 0 THEN 'AMBIGUOUS_MULTIPLE_WITH_HISTORY'
    WHEN group_hist_count >= 1 AND hist_total > 0 THEN 'CANONICAL_KEEP_UPDATE'
    WHEN group_hist_count = 0 AND pick_rank = 1 THEN 'CANONICAL_KEEP_UPDATE_NO_HISTORY'
    ELSE 'GENUINE_DUPLICATE_ZERO_HISTORY'
  END AS classification,
  CASE
    WHEN group_hist_count > 1 AND hist_total > 0 THEN 'Multiple rows for this ERP part carry real history ('||group_hist_count||' rows) - needs manual pick'
    WHEN group_hist_count >= 1 AND hist_total > 0 THEN 'Matches ERP shrp_part_code='||erp_shrp_code_raw||'; holds real history (prod='||prod_entries||' bags='||bags||' mach='||mach_assign||' fpa='||fpa||')'
    WHEN group_hist_count = 0 AND pick_rank = 1 AND group_size > 1 THEN 'Matches ERP part but zero history anywhere in group of '||group_size||'; kept as sole representative (lowest id / already SHRP-coded)'
    WHEN group_hist_count = 0 AND pick_rank = 1 THEN 'Matches ERP part, zero history, sole matching row - kept as-is'
    ELSE 'Zero history; duplicate of id holding same ERP part (see canonical row for this ERP part)'
  END AS reason
FROM (
  SELECT mr.*, c_key AS erp_key, canon_shrp AS erp_shrp_code_raw FROM matched_ranked mr
) x
UNION ALL
SELECT
  id, part_code, NULL AS erp_shrp_code, customer_part_no, active,
  cavity_count, NULL AS canon_cavity, part_weight_g, NULL AS canon_weight,
  hist_total,
  CASE WHEN hist_total > 0 THEN 'AMBIGUOUS_NO_ERP_MATCH_HAS_HISTORY' ELSE 'GENUINE_DUPLICATE_ZERO_HISTORY' END AS classification,
  CASE WHEN hist_total > 0 THEN 'No matching customer_part_no in ERP master, but has real history (prod='||prod_entries||' bags='||bags||' mach='||mach_assign||' fpa='||fpa||') - needs manual decision'
       ELSE 'No matching customer_part_no in ERP master and zero history - likely stray/test row' END AS reason
FROM unmatched
