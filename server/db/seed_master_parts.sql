...
DECLARE
  ...
  v_part_code TEXT;
BEGIN
  -- Temporary table containing the complete 77 parts
  CREATE TEMP TABLE IF NOT EXISTS temp_master_seed (
    shrp_part_code TEXT PRIMARY KEY,
    ...
    batch_part_code TEXT
  ) ON COMMIT DROP;

  TRUNCATE temp_master_seed;

  INSERT INTO temp_master_seed (
    shrp_part_code, customer_part_no, part_weight_g, trim_required,
    inspection_required, packing_required, dispatch_required, tolerance_pct,
    standard_pack_qty, cavity_count, unit_weight_g, batch_part_code
  ) VALUES    ('LBB', 'HC442L3LBB01', 6.07, FALSE, TRUE, TRUE, TRUE, 2, 250, 4, 31, '33'),
    ...
    ('NDGAA', 'FC1F2NDGAA02', 2, FALSE, TRUE, TRUE, TRUE, 2, 750, 6, 16.7, '47'),
    ('QVEAC', 'HC442QVEAC01', 1.21, FALSE, TRUE, TRUE, TRUE, 2, 1000, 8, 16, '39'),
    ('QVEBC', 'HC442QVEBC01', 1.5, FALSE, TRUE, TRUE, TRUE, 2, 1000, 8, 19, '40'),
    ('BH-DIA 16 - MAA', 'PL FC1E4K5MAA - 00', 1.24, FALSE, TRUE, TRUE, TRUE, 2, 1000, 6, 13, 'B1'),
    ('BH-DIA 12 -MBA', 'PL FC1E4K5MBA - 00', 1.36, FALSE, TRUE, TRUE, TRUE, 2, 750, 6, 14, 'B2'),
    ('BH-DIA 8', 'FC1P4L1E1A01', 0.78, TRUE, TRUE, TRUE, TRUE, 2, 1500, 6, 7, 'B3'),
    ('HR241', 'HR241G6C1A01', 0, FALSE, TRUE, TRUE, TRUE, 2, 400, 6, 18.5, 'B4'),
    ('OUTLET', 'PLHC442G6C1B', 1.44, TRUE, TRUE, TRUE, TRUE, 2, 1000, 4, 8.5, 'B5'),
    ('INLET', 'PLHC442G6C1A', 1.44, TRUE, TRUE, TRUE, TRUE, 2, 1000, 4, 8.5, 'B6'),
    ('PUNE B', 'FC1E1BAE1D01', 2.36, FALSE, TRUE, TRUE, TRUE, 2, 400, 6, 19.5, 'P1'),
    ('PUNE S', 'HR241BAE1B01', 1, FALSE, TRUE, TRUE, TRUE, 2, 800, 6, 11.5, 'P2'),
    ('F390', 'F390-QQDC-A02', 0, FALSE, TRUE, TRUE, TRUE, 2, 1000, 0, 0, 'W1'),
    ('LMF', 'WC-SCP-ECC21-LMF01', 2.05, TRUE, TRUE, TRUE, TRUE, 2, 650, 2, 6.5, 'W10'),
    ('LMM', 'WC-SCP-eCC21-Lmm01', 0, TRUE, TRUE, TRUE, TRUE, 2, 650, 0, 0, 'W11'),
    ('SMF', 'WC-SCP-ECC21-SMF01', 2.73, TRUE, TRUE, TRUE, TRUE, 2, 500, 2, 6.5, 'W12'),
    ('SMM', 'WC-SCP-eCC21-Smm01', 2.48, TRUE, TRUE, TRUE, TRUE, 2, 500, 2, 5, 'W13'),
    ...
    ('KQ NEW', 'F442-KQ', 0, TRUE, TRUE, TRUE, TRUE, 2, 1000, 0, 0, 'W3'),
    ('SHRP-T10', 'SHRP-T10', 0, TRUE, TRUE, TRUE, TRUE, 2, 2000, 0, 0, 'W4'),
    ('SHRP-T12', 'SHRP-T12', 1.62, TRUE, TRUE, TRUE, TRUE, 2, 750, 6, 11, 'W5'),
    ('SHRP-T3', 'SHRP-T3', 0, TRUE, TRUE, TRUE, TRUE, 2, 600, 0, 0, 'W6'),
    ('SHRP-T7', 'SHRP-T7', 0, TRUE, TRUE, TRUE, TRUE, 2, 2000, 0, 0, 'W7'),
    ('SHRP-T8', 'SHRP-T8', 2.21, TRUE, TRUE, TRUE, TRUE, 2, 500, 2, 5.5, 'W8'),
    ('ATBAB', 'F442-ATBAB02', 3.63, TRUE, TRUE, TRUE, TRUE, 2, 300, 2, 8.5, 'W18'),
    ('DA', 'WC-SCP-ECC21-DA01', 2.37, TRUE, TRUE, TRUE, TRUE, 2, 500, 2, 5.5, 'W9'),
    ('LAC Green', 'HC442L3LAC01', 4.44, FALSE, TRUE, TRUE, TRUE, 2, 400, 0, 0, 'W17'),
    ('R101', 'R101WC9AA01', 0, FALSE, TRUE, TRUE, TRUE, 2, 2000, 0, 0, 'B7'),
    ('T40', 'RM-19D935-BA', 1.87, FALSE, TRUE, TRUE, TRUE, 2, 750, 4, 8, '41'),
    ('VK GR -6', 'VK Grommet', 0, TRUE, TRUE, TRUE, TRUE, 2, 1500, 0, 0, '0'),
    ('VP6T', 'VP6TLU-11N087-AA', 0, TRUE, TRUE, TRUE, TRUE, 2, 1000, 16, 0, 'C1'),
    ('9AA', 'VPR230WC9AA01', 0, TRUE, TRUE, TRUE, TRUE, 2, 400, 0, 0, 'B8'),
    ('9AB', 'VPR230-WC9AB-01', 3.1, FALSE, TRUE, TRUE, TRUE, 2, 300, 4, 13.6, 'B9'),
    ('SYRINGE CAP', NULL, 0, TRUE, TRUE, TRUE, TRUE, 2, 1500, 12, 0, 'O1');

  -- Upsert logic iterating through all 77 seed parts
  FOR v_rec IN SELECT * FROM temp_master_seed LOOP
    -- Determine default unique part_code for the part
    IF v_rec.shrp_part_code = 'F364 GS' THEN
      v_part_code := 'F364-CB5AA-01-GS';
    ELSIF v_rec.shrp_part_code = 'F364 16C' THEN
      v_part_code := 'F364-CB5AA-01';
    ELSIF v_rec.shrp_part_code = 'QQVBA Y' THEN
      v_part_code := 'HC443QQVBA02-Y';
    ELSIF v_rec.shrp_part_code = 'QQVBA W' THEN
      v_part_code := 'HC443QQVBA02';
    ELSIF v_rec.shrp_part_code = 'LAC Green' THEN
      v_part_code := 'HC442L3LAC01-GRN';
    ELSIF v_rec.shrp_part_code = 'LAC Blue' THEN
      v_part_code := 'HC442L3LAC01';
    ELSIF v_rec.shrp_part_code = 'SYRINGE CAP' THEN
      v_part_code := 'SYRINGE-CAP';
    ELSE
      v_part_code := COALESCE(NULLIF(v_rec.customer_part_no, ''), v_rec.shrp_part_code);
    END IF;

    -- Look for existing part by shrp_part_code first
    v_part_id := NULL;
    SELECT id INTO v_part_id FROM parts WHERE shrp_part_code = v_rec.shrp_part_code LIMIT 1;
    ...
    -- If not found and customer_part_no is unique (not one of the duplicate pairs)
    IF v_part_id IS NULL AND v_rec.customer_part_no IS NOT NULL 
       AND v_rec.customer_part_no NOT IN ('F364-CB5AA-01', 'HC443QQVBA02', 'HC442L3LAC01') THEN
      SELECT id INTO v_part_id FROM parts WHERE customer_part_no = v_rec.customer_part_no OR part_code = v_rec.customer_part_no LIMIT 1;
    END IF;

    IF v_part_id IS NOT NULL THEN
      -- Update existing part in-place, preserving id and foreign keys
      UPDATE parts SET
        shrp_part_code = v_rec.shrp_part_code,
        customer_part_no = COALESCE(v_rec.customer_part_no, customer_part_no, v_rec.shrp_part_code),
        part_name = COALESCE(NULLIF(part_name, ''), v_rec.shrp_part_code),
        part_weight_g = v_rec.part_weight_g,
        cavity_count = v_rec.cavity_count,
        unit_weight_g = v_rec.unit_weight_g,
        standard_pack_qty = v_rec.standard_pack_qty,
        batch_part_code = v_rec.batch_part_code,
        trim_required = v_rec.trim_required,
        inspection_required = v_rec.inspection_required,
        packing_required = v_rec.packing_required,
        dispatch_required = v_rec.dispatch_required,
        tolerance_pct = v_rec.tolerance_pct,
        active = TRUE
      WHERE id = v_part_id;
    ELSE
      -- Insert new part
      -- Ensure v_part_code does not collide with any existing part
      IF EXISTS (SELECT 1 FROM parts WHERE part_code = v_part_code) THEN
        v_part_code := v_rec.shrp_part_code;
      END IF;
      ...
      INSERT INTO parts (
        ...
        v_part_code,
        v_rec.shrp_part_code,
        v_rec.shrp_part_code,
        COALESCE(v_rec.customer_part_no, v_rec.shrp_part_code),
        v_rec.part_weight_g,
        v_rec.cavity_count,
        v_rec.unit_weight_g,
        v_rec.standard_pack_qty,
        v_rec.batch_part_code,
        v_rec.trim_required,
        v_rec.inspection_required,
        v_rec.packing_required,
        v_rec.dispatch_required,
        v_rec.tolerance_pct,
        0,
        TRUE
      );
  ...
  RAISE NOTICE 'Seed master parts completed successfully: 77 parts processed.';
END $seed$;